...
mujeres_medias<-uso_tiempo_2016|>
  ...
  mutate(media_trabajo_pago=mean(trabajo_pago,na.rm=TRUE))|>
  mutate(media_trabajo_domestico_no_pago=mean(trabajo_domestico_no_pago,na.rm = TRUE))|>
  mutate (media_tareas_cuidados=mean(tareas_cuidados,na.rm = TRUE))
```
...
```{r}
#|fig.cap="Promedio de horas que las mujeres de 18 o más dedican a tareas de cuidado según su estado ocupacional"
...
uso_tiempo_2016|>
  ...
  group_by(sexo)|>
  summarise(media_trabajo_domestico_no_pago=mean(trabajo_domestico_no_pago,na.rm = TRUE))|>
  ggplot(mapping = aes(media_trabajo_domestico_no_pago,sexo,fill=sexo))+
geom_col(colour="black")
